...
def build_constraint_table(constraints, agent):
    ...
    for constraint in constraints:
        if constraint['timestep'] not in constraint_table.keys():
        ...
        for location in constraint_table[next_time]:
            if len(location) == 1:
                if location[0] == next_loc:
                    return True
            if len(location) == 2:
                if location[0] == curr_loc and location[1] == next_loc:
                    return True
        return False
    else:
        return False
...
def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    ...
    while len(open_list) > 0:
        ...
        for dir in range(5):
            ...
            if my_map[child_loc[0]][child_loc[1]] and not is_constrained(curr['loc'], child_loc, curr['timestep']+1, constraint_table):
                continue
            child = {'loc': child_loc,
                     'g_val': curr['g_val'] + 1,
